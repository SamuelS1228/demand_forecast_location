...
def assign_mileage_band(miles: pd.Series, bands: pd.DataFrame) -> pd.Series:
    bins = list(bands["Min Miles"]) + [bands["Max Miles"].iloc[-1] + 1]
    labels = list(bands["Mileage Band"])
    return pd.cut(miles, bins=bins, labels=labels, right=False, include_lowest=True).astype("object").fillna("Above Max Mileage")
...
def run_forecast(
    ...
    age_vmt["Mileage Band"] = assign_mileage_band(age_vmt["Cumulative Miles"], bands)
